...
def topCounter(request):
  ...
  return JsonResponse({'data': {
     ...
     'month_profits':month_profits if month_profits else 0,
     'year_profits':year_profits if year_profits else 0,
  }})

# @login_required(login_url='/login')
# @permission_required("dashboard.view_dashboard",login_url='/login',raise_exception=True)
def getProfitsPage(request):
  current_year = datetime.now().year
  #return HttpResponse(current_year)
  datepicker = request.GET.get('datepicker', current_year)
  profits = Profit.objects.filter(created_at__year=datepicker).annotate(
  ...
  # if datepicker:
  #profits.filter(price55=50)
  #filter(created_at__year=datepicker)    
  results_with_names = []
  ...
  # }
  context = {'month':month,'price':price}
  return render(request,'partials/_chart.html',context)

# def getProfits(request):
#   datepicker = request.GET.get('datepicker', '2025')
